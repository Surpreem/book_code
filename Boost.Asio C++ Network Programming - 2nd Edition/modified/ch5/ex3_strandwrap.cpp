...
int main()
{
    ...
    io_srv->post(strand.wrap(std::bind(print, 1)));
    io_srv->post(strand.wrap(std::bind(print, 2)));

    //std::this_thread::sleep_for(100ms);
    io_srv->post(strand.wrap(std::bind(print, 3)));
    io_srv->post(strand.wrap(std::bind(print, 4)));

    //std::this_thread::sleep_for(100ms);
    io_srv->post(strand.wrap(std::bind(print, 5)));
    io_srv->post(strand.wrap(std::bind(print, 6)));
